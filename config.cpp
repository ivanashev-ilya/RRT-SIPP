...
bool Config::getConfig(const char *FileName)
{
    ...
    getValueFromText(algorithm, CNS_TAG_TL, "int", &maxTime);
    getValueFromText(algorithm, CNS_TAG_ME, "double", &maxEdgeLength);
    getValueFromText(algorithm, CNS_TAG_SEED, "int", &seed);
    ...
    if (value == CNS_SP_ST_RRT) {
        N = 4;
        SearchParams = new double[N];
        SearchParams[CN_SP_ST] = CN_SP_ST_RRT;
    }
    else if (value == CNS_SP_ST_RRT_SIPP) {
        N = 4;
        SearchParams = new double[N];
        SearchParams[CN_SP_ST] = CN_SP_ST_RRT_SIPP;
    }
    else if (value == CNS_SP_ST_RRT_STAR_SIPP) {
        N = 4;
        SearchParams = new double[N];
        SearchParams[CN_SP_ST] = CN_SP_ST_RRT_STAR_SIPP;
    }
    else {
        std::cout << "Error! Value of '" << CNS_TAG_ST << "' tag (algorithm name) is not correctly specified."
                  << std::endl;
        std::cout << "Supported algorithm's names are: '" << CNS_SP_ST_RRT << "'." << std::endl;
        return false;
    }
