...
{
    return obstacles.getObstacles(fileName);
}
...
void Mission::createSearch()
{
    ...
    if (config.SearchParams[CN_SP_ST] == CN_SP_ST_RRT) {
        search = new RRT(config.maxEdgeLength);
    } else if (config.SearchParams[CN_SP_ST] == CN_SP_ST_RRT_SIPP) {
        search = new RRT_SIPP(config.maxEdgeLength);
    } else if (config.SearchParams[CN_SP_ST] == CN_SP_ST_RRT_STAR_SIPP) {
        search = new RRT_STAR_SIPP(config.maxEdgeLength);
    }
...
void Mission::fillCAT() {
    for (int i = 0; i < obstacles.getNumberOfObstacles(); ++i) {
        const auto& sections = obstacles.getSections(i);
        CAT.addObstaclePath(sections, map);
    }
}
...
void Mission::startSearch()
{
    ...
    sr = search->startSearch(map, config, CAT, rng);
}
...
void Mission::saveSearchResultsToLog()
{
    ...
    if (sr.pathfound) {
        logger->writeToLogPath(*sr.path);
        logger->writeToLogDynamicObstacles(obstacles);
        //logger->writeToLogMap(map, *sr.path);
    } else
