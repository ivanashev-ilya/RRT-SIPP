...
bool Map::getMap(const char *FileName)
{
    ...
    distI = std::uniform_real_distribution<>(0.5, width - 0.5);
    distJ = std::uniform_real_distribution<>(0.5, height - 0.5);
...
double Map::getRandom_i(std::mt19937 &rng) {
    return distI(rng);
}

double Map::getRandom_j(std::mt19937 &rng) {
    return distJ(rng);
}
